...
def det_add():
    ...
    for sift in sifts:
        if (sift[2] != '') & (sift[2] != '未作成'):
            ...
            event["start"]["dateTime"] = set_timezone(year[year_sel], monthDay[0], monthDay[1], start[0], start[1])
            event["end"]["dateTime"] = set_timezone(year[year_sel], monthDay[0], monthDay[1], end[0], end[1])
            events.append(event)
    driver.close()
    return events
...
def set_timezone(year, month, day, hour, minute):
    ...
    # naiveのdatetime作成
    datetime_naive = datetime.datetime(int(year), int(month), int(day), int(hour), int(minute))
